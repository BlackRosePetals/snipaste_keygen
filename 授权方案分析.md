...
```json
{
  ...
  "hwi": {
    "machineid": "DEFB-9113F596-9DD67DF1",
    "platform": 1,
    "domain": ""
  }
}
...
| 形态 | 示例 | 用途 |
|------|------|------|
| 界面展示码 | `3-6NHNW-WHKWH-6PE73-W66TH` | 给用户看（字母映射后） |
| 比较用明文 | `DEFB-9113F596-9DD67DF1` | validate 实际比较的值 |
| 字段存储态 | encode 后的字节 | JSON 内部存储（客户端自行编解码） |

**结论：`make_json` 的 `hwi.machineid` 直接填比较用明文 `DEFB-9113F596-9DD67DF1` 即可（JSON 层是明文，客户端内部自行 encode/decode）。**
...
- 本机方案：`len = 9`，得 `DEFB-9113F596-9DD67DF1`。
...
dom 字段填本机工作组明文 `"SENSE"` 即可（走 way1）。
...
```
1. dom        = get_workgroup()                    # NetWkstaGetInfo → 'SENSE'
2. machineid  = gen_expected_machineid(guid, 9)    # blake2s[-9:] → DEFB-9113F596-9DD67DF1
